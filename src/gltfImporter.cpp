...
void GLTFModel::processGLTFModel(tinygltf::Model &model)
{
	for (const auto &mesh : model.meshes)
	{
		...
		{
			std::pair<std::vector<float>, std::pair<glm::vec3, glm::vec3>> posAndBB = processPosAttrib(primitive, mesh, model);
...
const std::pair<std::vector<float>, std::pair<glm::vec3, glm::vec3>> GLTFModel::processPosAttrib(const tinygltf::Primitive &primitive,
