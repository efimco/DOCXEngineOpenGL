...
UIManager::UIManager(GLFWwindow *window, Camera &camera) : camera(camera),
														   window(window)
{
	ImGui::CreateContext();
...
void UIManager::draw(float deltaTime)
{
	...
	ImGui::NewFrame();
	ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_NoBackground;
...
void UIManager::showCameraTransforms()
{
	...
	ImGui::Text("Default Pos: %.1f, %.1f, %.1f", camera.defaultCameraMatrix[0].x, camera.defaultCameraMatrix[0].y, camera.defaultCameraMatrix[0].z);
	ImGui::SameLine();
	ImGui::Text("Default Rot: %.1f, %.1f", camera.defaultCameraRotation[0], camera.defaultCameraRotation[1]);
	ImGui::Text("Default Front: %.1f, %.1f, %.1f", camera.defaultCameraMatrix[1].x, camera.defaultCameraMatrix[1].y, camera.defaultCameraMatrix[1].z);
	ImGui::SameLine();
...
void UIManager::showLights()
{
	...
	{
		ImGui::PushID(i);
		ImGui::Text("Light: %d %s ", i, (SceneManager::getLights()[i].type == 1) ? "directional" : (SceneManager::getLights()[i].type == 2) ? "spot"
																																			: "point");
